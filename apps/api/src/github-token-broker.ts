...
export function createGitHubAppTokenBroker(options: CreateGitHubAppTokenBrokerOptions): GitHubTokenBroker {
  ...
  return {
    async mintInstallationToken(input): Promise<GitHubInstallationTokenResult> {
      ...
      if (!response.ok) {
        return { ok: false, status: response.status, error: readGitHubError(body, response.status) };
      }
      if (typeof body.token !== "string" || !body.token.trim()) {
        return { ok: false, status: 502, error: "github_token_response_missing_token" };
      }

      return {
        ok: true,
        token: {
          envName: config.tokenEnvName,
          value: body.token,
