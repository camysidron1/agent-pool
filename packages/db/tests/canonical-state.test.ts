...
import { TASK_PRIORITY_SCHEMA_MIGRATION_ID, createCanonicalStateServices, migrateWebSandboxDatabase } from "../src";
...
describe("canonical backend state acceptance", () => {
  test("empty DB migrates to latest schema and exposes core tables", () => {
    ...
    try {
      ...
      expect(result.applied.at(-1)?.id).toBe(TASK_PRIORITY_SCHEMA_MIGRATION_ID);
